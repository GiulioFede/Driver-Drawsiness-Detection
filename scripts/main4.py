...
MIN_NUMBER_OF_FRAMES_FOR_EAR_TUNING = FPS_CAMERA*15
FRAME_WIDTH = 800
FRAME_HEIGHT = 600
""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
class RPERCLOS(object):
    ...
    def update_rperclos(self, current_frame_state, current_timestamp):
        ...
        #time_ base is computed with respect to oldest timestamp in wake_up buffer
        if len(self.wake_up_buffer) >= WAKE_UP_BUFFER_LENGHT:
        # have o fill wakeup buffer
            oldest_tstamp = self.wake_up_buffer[0]
            time_base = current_timestamp - oldest_tstamp
            norm_avg_wakeupframes = self.moving_average_wake_up_buffer - oldest_tstamp
            #should stay between 0 and 1
            wakeup_coefficent = norm_avg_wakeupframes/(time_base)
            #print(wakeup_coefficent)
            self.rperclos = base_perclos * wakeup_coefficent
        else:
        # empty
            self.rperclos = base_perclos
...
class DrawsinessEvaluator(object):
    ...
    def face_detection(self,detector):
        ...
        #print("DIMENSIONI IMMAGINE PRIMA", self.last_frame.shape)
        #res_frame = imutils.resize(self.last_frame, width=1920)
        res_frame = self.last_frame
        #print("DIMENSIONI IMMAGINE DOPO", res_frame.shape)
        #print("tempo durata resize", (time.time()-s_time))
    ...
    def RPERCLOS_application(self):
        ...
        if self.isEARFineTuned == False:
            ...
            cv2.putText(self.last_frame, "EAR fine tuning... EAR= {:.2f}".format(0.8*self.EAR_THREASHOLD), (170, 260), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        ...
        #draw EAR threshold
        cv2.putText(self.last_frame, "EAR threshold: {:.2f}".format(self.EAR_THREASHOLD), (FRAME_WIDTH-400, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        ...
        self.last_frame =cv2.rectangle(self.last_frame, (5,5), (FRAME_WIDTH-165, FRAME_HEIGHT-127), color, 4)
    ...
    def edit_final_image(self):
        ...
        self.last_frame = cv2.putText(self.last_frame, "EAR: {:.2f}".format(self.ear), (FRAME_WIDTH-290, 30),
        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
...
#per il test
columns = ['face_detection_time', 'landmark_prediction_time','ear_evaluation_time','percl_estimation_time','fps']
...
while True:
        try:
            ...
            for face in face_frames: #this 'for' is here because multiple frames could be detected, in this case we have to consider just the driver one
                ...
                time3 = time.time() -s_time
                
                #PERCL ESTIMATION AND ALERT LEVEL DETECTION
                s_time = time.time()
                drawsiness_videodetector.RPERCLOS_application()
                time4 = time.time() -s_time


                #registro nel dataframe
                if(j>5 and i<2100):
                    df.loc[i] = pd.Series([time1,time2,time3,time4,str((j / (time.time()-initial_time)))], columns )
                elif i==2100:
                    #salvo
                    df.to_csv("test_asus_rog_320x240.csv")
                i=i+1
